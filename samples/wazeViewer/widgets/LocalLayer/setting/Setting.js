    ...
    return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
      ...
      startup: function() {
        ...
        aspect.before(this, 'destroy', function(){
          //This updates the setting's layerinfo object with the stuff created by the locallayer widget operating on the map.
          if (LayerInfos.getInstanceSync()){
            if (this.map.updatedLayerInfos){
              LayerInfos.getInstanceSync()._tables = this.map.updatedLayerInfos._tables;  
              LayerInfos.getInstanceSync()._initTablesInfos();
              this.map.itemInfo.itemData.operationalLayers = this.map.updatedLayerInfos;
            }
            aspect.after(LayerInfos,"getInstance",lang.hitch(this,function(mapItem,mapId){
              var deferred = new Deferred()
              deferred.resolve(this.map.updatedLayerInfos);
      ...
      setConfig: function(config) {
        ...
        if(config.replaceLayers === false){
          this.replaceCbx.setValue(false)
        }else{
          this.replaceCbx.setValue(true)
        }
      ...
      _bindEvents: function() {
        ...
        this.own(on(this.btnAddCustomLayer,'click',lang.hitch(this,function(){
          var args = {
             config:{type:'Custom'}
          };
          this.popupState = 'ADD';
      ...
      _onCustomEditOk: function() {
        var layerConfig = this.popupcustomedit.getConfig();
        console.info(layerConfig);
      ...
      _onCustomEditClose: function() {
        var layerConfig = this.popupcustomedit.getConfig();
        if(this.popupState === 'ADD'){
          this.LayersTable.deleteRow(layerConfig[1]);
        }
        this.popupcustomedit = null;
        this.popup = null;
      },

      _openCustomEdit: function(title, tr) {
        this.popupcustomedit = new CustomLayerEdit({
          nls: this.nls,
          config: tr.singleLayer || {},
          tr: tr,
          map: this.map
        ...
        this.popup = new Popup({
          ...
          buttons: [{
            label: this.nls.ok,
            key: keys.ENTER,
            onClick: lang.hitch(this, '_onCustomEditOk')
          }, {
            label: this.nls.cancel,
            key: keys.ESCAPE
          }],
          onClose: lang.hitch(this, '_onCustomEditClose')
        });
        html.addClass(this.popup.domNode, 'widget-setting-popup');
        this.popupcustomedit.startup();
      },
